...
$result = mysqli_query($conn, "SELECT * FROM clientes ORDER BY id DESC LIMIT " . $inicio . "," . $tamano_pagina);
?>
...
<col style="width: 70px">
  ...
  <?php //Mostramos los registros
	...
	echo '<tr class="' . $estado . '" style="height: 15px !important;">';
	echo '<td>' . $row["id"] . '</td>';
...
<?php 
	...
	//Mostrar paginacion
	if ($total_paginas > 1) {
		echo '<div class="pag"><a href="gestion.php?pag=1"><<</a></div>';
		if ($pagina != 1) {
			echo '<div class="pag"><a href="gestion.php?pag=' . ($pagina - 1) . '"><</a></div>';
		}
		//Muestro paginas
		for ($i = $pagina_desde; $i <= $pagina_hasta; $i++) {
			...
			echo '<div class="pag">' . $pagina . '</div>';
				...
				echo '<div class="pag"><a href="gestion.php?pag=' . $i . '">' . $i . ' </a></div>';
			}
		}
		if ($pagina != $total_paginas) {
			echo '<div class="pag"><a href="gestion.php?pag=' . ($pagina + 1) . '">></a></div>';
		}
		echo '<div class="pag"><a href="gestion.php?pag=' . $total_paginas . '">>></a></div>';
	}
